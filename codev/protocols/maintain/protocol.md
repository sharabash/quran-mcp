...
MAINTAIN is a periodic maintenance protocol for keeping codebases healthy. It runs as a **strict porch protocol** with sequential phases and 2-tool consultation (`/codex:review` + `/gemini`) at each phase.
...
2. CLEAN: Remove identified cruft
    ↓ (2-tool review + build/test checks)
...
Each phase goes through build-verify cycles with 2-tool consultation before proceeding.
...
| "Consult tool queries AI models" | "Consultation uses native Claude Code plugins (`/codex:review`, `/codex:adversarial-review`, `/gemini`). Each plugin is invoked as a slash command with arguments, and results are returned inline." |
...
## Consultation Review (Before PR)

After completing all tasks and validation, run both reviews before PR:

1. `/codex:review --base main` — Review all maintenance changes against main
2. `/gemini --dirs .` — Holistic review of maintenance changes
...
Only create the PR after the consultation review passes.
...
| Plan | No |
| Review | No |
| 2-Tool Consultation (`/codex:review` + `/gemini`) | **Yes** (before creating PR) |
