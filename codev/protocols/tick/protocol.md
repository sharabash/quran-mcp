...
2. **Multi-Agent Consultation** (MANDATORY):
   - Run `/codex:review --base <pre-tick-ref>` and `/gemini --files <changed-files>`
...
| Multi-agent consultation (2-tool: `/codex:review` + `/gemini`) | Throughout | End only (review) |
...
5. **Review** (1 minute):
   ...
   - Runs 2-tool consultation (`/codex:review` + `/gemini`)
