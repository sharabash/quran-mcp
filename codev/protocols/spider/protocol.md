...
>
> Each phase has one build-verify cycle with 2-tool consultation (`/codex:review` + `/gemini`).
...
- Claude Code plugin `/codex:review` (GPT-5 Codex review tool)
- Claude Code plugin `/gemini` (Gemini CLI integration)
- Verify availability: `/codex:setup` and `/gemini --help`
...
**DEFAULT REVIEWERS:**
- **GPT-5 Codex** (primary): Architecture, feasibility, code quality
  - For code review: `/codex:review --base <ref>`
  - For spec/plan/design review: `/codex:adversarial-review` with focus text describing what to challenge
- **Gemini Pro** (secondary): Completeness, edge cases, alternative approaches
  - For all review types: `/gemini` with relevant files via `--files` or `--dirs`
...
- DEFAULT: MANDATORY consultation via `/codex:review` (or `/codex:adversarial-review` for document artifacts) and `/gemini` at EVERY checkpoint
...
- Both tools support `--background` mode; use `--wait` if you need to block on results
...
**The SPIR Model**:
- **S - Specify**: Write specification with 2-tool review (`/codex:adversarial-review` + `/gemini`) → Gate: `spec-approval`
- **P - Plan**: Write implementation plan with 2-tool review (`/codex:adversarial-review` + `/gemini`) → Gate: `plan-approval`
- **I - Implement**: Execute each plan phase with build-verify cycle (one cycle per phase)
- **R - Review**: Final review and PR preparation with 2-tool review (`/codex:review` + `/gemini`)

Each phase follows a build-verify loop: build the artifact, then verify with 2-tool consultation (`/codex:review` + `/gemini`).
...
4. Multi-agent review via `/codex:adversarial-review` and `/gemini`
...
6. **Expert Consultation (DEFAULT - MANDATORY)**
   - **First Consultation** (after initial draft):
     - MUST run `/codex:adversarial-review` and `/gemini --files <artifact>`
     - Focus: Problem clarity, solution completeness, missing requirements
     - Update specification with ALL feedback from both tools
     - Document changes in "Consultation Log" section of the spec
   - **Second Consultation** (after human comments):
     - MUST run `/codex:adversarial-review` and `/gemini --files <artifact>` again
     - Focus: Validate changes, ensure alignment
     - Final specification update with both tools' input
...
3. Multi-agent review via `/codex:adversarial-review` and `/gemini`
...
3. **Expert Review (DEFAULT - MANDATORY)**
   - **First Consultation** (after plan creation):
     ...
     - Update plan with ALL feedback from both tools
   - **Second Consultation** (after human review):
     - MUST run `/codex:adversarial-review` and `/gemini --files <artifact>` again
     - Focus: Validate adjustments, confirm approach
     - Final plan refinement with both tools' input
...
2. **Verify** - 2-tool consultation (`/codex:review` + `/gemini`)
...
- MUST run `/codex:review --base <ref>` and `/gemini --files <changed-files>` after implementation
- Focus: Code quality, patterns, security, best practices
- Update code based on feedback from BOTH tools before proceeding
...
**Expert Consultation (DEFAULT - MANDATORY)**:
- MUST run `/codex:review --base <ref>` and `/gemini --files <changed-files>` for test defense review
- Focus: Test coverage completeness, edge cases, defensive patterns, test strategy
- Write additional defensive tests based on feedback from BOTH tools
...
4. **Expert Consultation Before User Evaluation** (MANDATORY - NO EXCEPTIONS)
   ...
   - **CRITICAL**: Re-run `/codex:review` and `/gemini` on the FIXED version to confirm clean results
   - Only proceed to user evaluation after both tools return clean
   - If either tool raises concerns, fix them FIRST
...
- Expert review notes from `/codex:review` and `/gemini`
...
- [ ] Expert consultation completed (`/codex:review` + `/gemini`)
...
- [ ] Expert consultation on tests completed (`/codex:review` + `/gemini`)
...
- [ ] Performance requirements satisfied
- [ ] Security standards maintained
- [ ] Expert consultation shows FINAL approval (both tools clean)
