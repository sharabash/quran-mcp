...
The SPIR protocol **requires** a final consultation with `/codex:review` and `/gemini` before submitting the PR. This ensures the complete implementation is reviewed.
...
Run final PR-readiness review (REQUIRED):
1. `/codex:review --base main` — Full diff review against main branch
2. `/gemini --dirs .` — Holistic codebase review
...
1. **Final consultation is MANDATORY** - Cannot skip `/codex:review` + `/gemini` reviews before PR
